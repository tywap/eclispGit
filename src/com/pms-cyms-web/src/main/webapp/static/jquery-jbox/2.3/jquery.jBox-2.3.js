	...
	a.jBox = function (b, c) {
		...
		if (c.showFade) {
			if ((d && a('iframe').length > 0x0) || a('object, applet').length > 0x0) {
				...
				j.push('<div id="jbox-fade" class="jbox-fade" style="background-color:black;position:absolute;"></div>');
			}
			// 解决框架内object和applet被遮住问题
		...
		j.push('<a class="jbox-close" title="' + a.jBox.languageDefaults.close + '" onmouseover="$(this).addClass(\'jbox-close-hover\');" onmouseout="$(this).attr(\'class\',\'jbox-close\');" style="position:absolute; display:block; cursor:pointer; top:' + (0x6 + c.border) + 'px; right:0; width:46px; height:31px;' + (c.showClose ? '' : 'display:none;') + '"></a>');
		if (g) {
...
jBoxConfig.defaults = {
    ...
    top: '70px', /* 窗口离顶部的距离,可以是百分比或像素(如 '100px') */
