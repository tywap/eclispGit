...
<ul style="list-style:none;float:left;" id="${input}Preview"></ul><c:if test="${!readonly}"><a style="float:left;" href="javascript:" onclick="${input}FinderOpen();" class="btn">${selectMultiple?'添加':'+'}</a></c:if>
<style>
	#htlImagePreview{
		overflow:hidden;
	}
	...
	function ${input}Preview(){
		...
		for (var i=0; i<urls.length; i++){
			if (urls[i]!=""){//<c:if test="${type eq 'thumb' || type eq 'images'}">
				...
				li += "<span class=\"set-cover\" onclick=\"setCover(this);\" style=\"position: absolute;bottom: 3px;background-color: rgba(0,0,0,.5);color: #fff;width: 88%;left: 3px;border-bottom-left-radius: 15px;border-bottom-right-radius: 15px;text-align: center;\">设为封面</span>";
