		...
		top.$.jBox.open("iframe:${ctx}/tag/treeselect?url="+encodeURIComponent("${url}")+"&module=${module}&checked=${checked}&extId=${extId}&isAll=${isAll}", "选择${title}", 300, 420, {
			ajaxData:{selectIds: $("#${id}Id").val()},buttons:{"确定":"ok", ${allowClear?"\"清除\":\"clear\", ":""}"关闭":true}, submit:function(v, h, f){
				if (v=="ok"){
					...
					if ($.selectOnClose!=undefined){			//回调函数
						$.selectOnClose(ids.join(",").replace(/u_/ig,""),names.join(","),idd);
					} 
					if ($.getStores!=undefined&&"${id}"=="storeIdId"){		
						
						$.getStores(nodes);
					}
					if ($.getAllChannelAndSource!=undefined&&"${id}"=="channelId"){		//客源渠道需要显示所有的值而不是根节点的值
						$.getAllChannelAndSource(nodes);
					}
