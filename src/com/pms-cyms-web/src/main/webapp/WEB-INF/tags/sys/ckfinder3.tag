...
<ul style="list-style:none;float:left;" id="${input}Preview"></ul><c:if test="${!readonly}"><a style="float:left;padding-top:10px;" href="javascript:" onclick="${input}FinderOpen();" class="btn">${selectMultiple?'添加':'+'}</a></c:if>
<style>
	#htlImagePreview{
		overflow:hidden;
	}
...
<script type="text/javascript">
	function ${input}FinderOpen(){//<c:if test="${type eq 'thumb'}"><c:set var="ctype" value="images"/></c:if><c:if test="${type ne 'thumb'}"><c:set var="ctype" value="${type}"/></c:if>
		...
		var url = "${ctxStatic}/ckfinder/ckfinder.html?type=${ctype}&start=${ctype}:${uploadPath}/"+year+"/"+month+
			"/&action=js&func=${input}SelectAction&thumbFunc=${input}ThumbSelectAction&cb=${input}Callback&dts=${type eq 'thumb'?'1':'0'}&sm=${selectMultiple?1:0}&a=4";
	...
	function ${input}SelectAction(fileUrl, data, allFiles){
		...
		$("#${input}").val(url);//</c:if>
		$.ajax({
			url:'${ctx}/wechat/htlStore/wxRoomTypeImgSave',
			data:{roomTypeId:"${roomTypeId}",url:url},
			success:function(){
				${input}Preview();
			},
			error:function(){
	...
	function setCover(_this){
		$(_this).parent('li').append("<span class='add-cover' style='position: absolute;top: 3px;left: 3px;width: 30px;text-align: center;background-color: red;color: #fff;'>封面</span>");
		$(_this).css({'display':'none'});
		$(_this).parent('li').siblings().children('.add-cover').remove();
		var picSrc = $(_this).prev().attr('src');
		top.$.publish("1",{testData:"hello",roomTypeId:"${param.roomTypeId}",picSrc:picSrc});
	}
	function ${input}mouseover(_this){
		if($(_this).children('.add-cover').html() == undefined){
			$(_this).children('.set-cover').css({'display':'block','cursor':'pointer'});
		}		
	}
	function ${input}mouseout(_this){
		$(_this).children('.set-cover').css({'display':'none'});
	}
	function ${input}Preview(){
		...
		for (var i=0; i<urls.length; i++){
			if (urls[i]!=""){//<c:if test="${type eq 'thumb' || type eq 'images'}">
				li = "<li onmouseover=\"${input}mouseover(this)\" onmouseout=\"${input}mouseout(this)\" style=\"position:relative; float:left;\"><img src=\""+urls[i]+"\" url=\""+urls[i]+"\" style=\"max-width:${empty maxWidth ? 200 : maxWidth}px;max-height:${empty maxHeight ? 200 : maxHeight}px;_height:${empty maxHeight ? 200 : maxHeight}px;border:0;padding:3px;\">";//</c:if><c:if test="${type ne 'thumb' && type ne 'images'}">
				li = "<li><a href=\""+urls[i]+"\" url=\""+urls[i]+"\" target=\"_blank\">"+decodeURIComponent(urls[i].substring(urls[i].lastIndexOf("/")+1))+"</a>";//</c:if>
				li += "<span class=\"set-cover\" onclick=\"setCover(this);\" style=\"position: absolute;bottom: 3px;background-color: rgba(0,0,0,.5);color: #fff;width: 88%;left: 3px;border-bottom-left-radius: 15px;border-bottom-right-radius: 15px;text-align: center;\">设fds为封面</span>";
				li += "&nbsp;&nbsp;<c:if test="${!readonly}"><a href=\"javascript:\" onclick=\"${input}Del(this);\"  style=\"position: absolute;top: -5px;right: 0;font-size: 1.5rem;background-color: red;color: #fff;border-radius: 50%;height: 25px;width: 25px;text-align: center;\">×</a></c:if></li>";
