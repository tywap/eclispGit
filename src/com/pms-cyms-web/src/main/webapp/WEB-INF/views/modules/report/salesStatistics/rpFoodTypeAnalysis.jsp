...
<body>
	<form:form id="searchForm" modelAttribute="dayOrder" action="${ctx}/report/rpFoodTypeAnalysis/list" method="post" class="breadcrumb form-search ">
		<ul class="ul-form">
			...
			<li><label>菜品小类：</label>
				<select id=foodTypeName name="foodTypeName" class="select-medium6" style="width: 100px;">
				<option value="">全部</option>
				<c:forEach items="${foodTypeNameList}" var="var" varStatus="vs">
                       	<option value="${var.id}" <c:if test="${var.id == param.foodTypeName }">selected ="selected"</c:if>> ${var.name}</option>
                    </c:forEach>
				</select>
				</li>
			<li>
	...
	<div id="my_div" class="fakeContainer first_div" style="overflow:auto;">
		...
		<thead>
			<tr id="my_tr">
				<th rowspan="2" >序号</th>
				<th rowspan="2" >分店</th>
				<th rowspan="2">菜品大类</th>
				<th rowspan="2">销售份数</th>
				<th rowspan="2">菜品小类</th>
				<th colspan="2">标准成本</th>
				<th colspan="4">折前销售</th>
				<th colspan="4">折后销售</th>
