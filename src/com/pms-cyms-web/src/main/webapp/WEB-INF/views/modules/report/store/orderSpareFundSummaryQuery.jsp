		...
		$(document).ready(function() {
			$("#wBtn").click(function(){
				top.$.jBox.confirm("确认要导出订单数据吗？","系统提示",function(v,h,f){
					if(v=="ok"){
						$("#searchForm").attr("action","${ctx}/report/ordSpareFund/ExportExcelSummary");
						$("#searchForm").submit();
						$("#searchForm").attr("action","${ctx}/report/ordSpareFund/querySpareFundSummary");
					}
				},{buttonsFocus:1});
				...
				<tbody>
					<c:forEach items="${ordSpareFundList}" var="order" >
						<tr style="cursor:pointer;">
							...
							<td>${fns:getNameByParamKey(order.payMethod,'payWay')}</td>
							<td>${order.amount}</td>
