...
<body>
	<form action="${ctx}/setting/ctFood/select" id="rentalsForm"
	...
	<div id="div_container">
		...
		style="width: 81%; float: left";>
				...
				<tbody id="ctFoodList">
					<c:forEach items="${ctFoodList}" var="re" varStatus="s">
						<tr>
							<td><label class="labels"> <input type="checkbox"
								class="weeks" value="${re.id}" id="weeks_${re.id}"  name="weeks"></label></td>
							<td>${re.foodBigType }</td>
							<td>${re.foodTypeName }</td>
							<td>${re.code }</td>
							<td>${re.name }</td>
							<td>${re.foodUnitName }</td>
							<td>${re.price }</td>
						</tr>
					</c:forEach>
				</tbody>
