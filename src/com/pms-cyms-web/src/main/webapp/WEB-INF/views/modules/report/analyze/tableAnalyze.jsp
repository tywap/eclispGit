...
<body>
	<form:form id="searchForm" modelAttribute="dayOrder" action="${ctx}/report/tableRunAnalyze/list" method="post" class="breadcrumb form-search ">
		<ul class="ul-form">
			...
			<li><label>用餐类型：</label>
				<select id="tableType" name="tableType" class="select-medium6" style="width: 100px;">
				<option value="">全部</option>
				<c:forEach items="${ctTableTypeList}" var="var" varStatus="vs">
                       	<option value="${var.id}" <c:if test="${var.id == param.tableType }">selected ="selected"</c:if>> ${var.name}</option>
                    </c:forEach>
				</select>
				</li>
			<li>
	...
	<div id="my_div" class="fakeContainer first_div" style="overflow:auto;">
		...
		<tbody>
		<c:forEach items="${listTbaleAnalyze}" var="ord" varStatus="ordStatus">
			<tr  orderId="${ord.id}" >
				<td>${ord.storeName}</td>
				<td>${ord.tableType}</td>
				<td>${ord.tableName}</td>
