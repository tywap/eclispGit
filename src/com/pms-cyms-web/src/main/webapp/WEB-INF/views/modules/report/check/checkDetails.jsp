...
<body>
	<form:form id="searchForm" modelAttribute="dayOrder" action="${ctx}/report/checkDetails/list" method="post" class="breadcrumb form-search ">
		<ul class="ul-form">
			<li>
			 	<label>分店：</label>
			...
			<li>
				...
				<select id="dateType" style="margin-right:0; width: 100px;" name="dateType"  class="select-medium6" onchange="changeDateCheck()" >
						<option value="1" <c:if test="${1==param.dateType}">selected</c:if>>开台时间</option>
						<option value="2" <c:if test="${2==param.dateType}">selected</c:if>>结账时间</option>
				</select>
			</li> 
			<li>
				<label></label>
			...
			<li><label>经营区域：</label>
				...
				<option value="">全部</option>
					<c:forEach items="${foorlList}" var="var" varStatus="vs">
                      	<option value="${var.id}" <c:if test="${var.id == param.floorName }">selected ="selected"</c:if>> ${var.name}</option>
                    </c:forEach>
				</select>
			</li>
			<li><label>客源：</label>
	...
	<div id="my_div" class="fakeContainer first_div" style="overflow:auto;">
		...
		<thead>
			<tr id="my_tr">
				<th rowspan="2" >分店</th>
				<th rowspan="2">状态</th>
				<th rowspan="2">开台时间</th>
				<th rowspan="2">操作员</th>
				<th rowspan="2">单号</th>
				<th rowspan="2">台号</th>
				<th rowspan="2">经营区域</th>
				<th rowspan="2">台号类别</th>
				<th rowspan="2">用餐人数</th>
				<th rowspan="2">客源</th>
				<th rowspan="2">会员/协议/三方单号</th>
				<th rowspan="2">销售员</th>
				<th rowspan="2">用餐类型</th>
				<th rowspan="2">用餐时段</th>
				<th colspan="${fn:length(foodTypeList)+1}">消费（按小类）</th>
				<th colspan="${fn:length(payWayList)+1}">收款（按支付方式）</th>
